...
public class TestExtractionOnlineLearningClausIE {
    ...
    public static void beforeAllTestMethods() {
		try {
			PatternFileTestUt.fileSwap(Filenames.JSON_PATTERNS, TestExtractionOnlineLearningClausIE.BACKUP_FILENAME);
			TestExtractionOnlineLearningClausIE.relationExtractorStatic = new RelationExtractor(false,true);
    ...
    public static void afterAllTestMethods() {    	
		try {
			relationExtractorStatic.turnOffClausIEServer();
			PatternFileTestUt.fileSwap(Filenames.JSON_PATTERNS, Filenames.JSON_PATTERNS+".testResult.json");
			PatternFileTestUt.fileSwap(TestExtractionOnlineLearningClausIE.BACKUP_FILENAME, Filenames.JSON_PATTERNS);
		} catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
		}
    ...
    public TestExtractionOnlineLearningClausIE(){
    	try {
			this.extractor = TestExtractionOnlineLearningClausIE.relationExtractorStatic;
		} catch (Exception e) {
			e.printStackTrace();
			fail(e.getMessage());
		}
		...
		try{
				 ...
				 if(relations!=null && !relations.isEmpty()){
						...
						for (Relation relation : relations) {
							System.out.print(relation.toString());
							System.out.println(" =>  ("+relation.getScore()+")");
							if(relation.isFromClausIE()) {
								System.err.println("Was extrated using clausIE!!!");
								fail("Was extrated using clausIE!!!");
							}
						}
						System.out.println();
				 }else{
